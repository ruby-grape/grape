...
describe Grape::Middleware::Versioner::Header do
  ...
  context "with a vendor name utilizing an underscore" do 
   before do
      @options = {
        version_options: {
          using: :header,
          vendor: 'long_vendor_name'
        }
      }
    end
    ...
    context 'api.type and api.subtype' do
      it 'sets type and subtype to first choice of content type if no preference given' do
        ...
        expect(env['api.subtype']).to eql 'vnd.long_vendor_name+xml'
    ...
    context 'api.format' do
      it 'is set' do
        status, _, env = subject.call('HTTP_ACCEPT' => 'application/vnd.long_vendor_name+json')
      ...
      ['v1', :v1].each do |version|
        context 'when version is set to #{version{ ' do
          ...
          it 'is set' do
            status, _, env =  subject.call('HTTP_ACCEPT' => 'application/vnd.long_vendor_name-v1+json')
            expect(env['api.format']).to eql 'json'
            expect(status).to eq(200)
          end
        end
    ...
    context 'api.vendor' do
      it 'is set' do
        status, _, env =  subject.call('HTTP_ACCEPT' => 'application/vnd.long_vendor_name')
      ...
      context 'when version is set' do
        ...
        it 'is set' do
          status, _, env =  subject.call('HTTP_ACCEPT' => 'application/vnd.long_vendor_name-v1')
          expect(env['api.vendor']).to eql 'vendor'
          expect(status).to eq(200)
        end
    ...
    context 'api.version' do
      ...
      it 'fails with 406 Not Acceptable if version is invalid' do
        expect { subject.call('HTTP_ACCEPT' => 'application/vnd.long_vendor_name-v2+json').last }.to raise_exception do |exception|
    ...
    context 'when multiple versions are specified' do
      ...
      it 'fails with another version' do
        expect { subject.call('HTTP_ACCEPT' => 'application/vnd.long_vendor_name-v3+json') }.to raise_exception do |exception|
          expect(exception).to be_a(Grape::Exceptions::InvalidAcceptHeader)
          expect(exception.headers).to eql('X-Cascade' => 'pass')
          expect(exception.status).to eql 406
          expect(exception.message).to include('API vendor or version not found')
  ...
  context "with a simple vendor name" do 
    before do
      @options = {
        version_options: {
          using: :header,
