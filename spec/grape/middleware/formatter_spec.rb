...
describe Grape::Middleware::Formatter do
  ...
  context 'accept header detection' do
    ...
    it 'ignores invalid quality rankings' do
      subject.call(Rack::PATH_INFO => '/info', 'HTTP_ACCEPT' => 'application/json;q=invalid,application/xml;q=0.5')
      expect(subject.env[Grape::Env::API_FORMAT]).to eq(:xml)
      subject.call(Rack::PATH_INFO => '/info', 'HTTP_ACCEPT' => 'application/xml;q=0.5,application/json;q=invalid')
      expect(subject.env[Grape::Env::API_FORMAT]).to eq(:xml)

      subject.call(Rack::PATH_INFO => '/info', 'HTTP_ACCEPT' => 'application/json;q=,application/xml;q=0.5')
      expect(subject.env[Grape::Env::API_FORMAT]).to eq(:json)

      subject.call(Rack::PATH_INFO => '/info', 'HTTP_ACCEPT' => 'application/json;q=nil,application/xml;q=0.5')
      expect(subject.env[Grape::Env::API_FORMAT]).to eq(:xml)
    end

    it 'parses headers with vendor and api version' do
      subject.call(Rack::PATH_INFO => '/info', 'HTTP_ACCEPT' => 'application/vnd.test-v1+xml')
      expect(subject.env[Grape::Env::API_FORMAT]).to eq(:xml)
    end
  ...
  context 'content-type' do
    ...
    it 'is set to closest generic for custom vendored/versioned without registered type' do
      _, headers, = subject.call(Rack::PATH_INFO => '/info', 'HTTP_ACCEPT' => 'application/vnd.test+json')
      expect(headers[Rack::CONTENT_TYPE]).to eq('application/json')
    end
  end
