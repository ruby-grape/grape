...
describe Grape::Validations::Validators::Base do
  describe 'freeze' do
    let(:validator) { Grape::Validations::Validators::PresenceValidator.new([:name], {}, true, nil, {}) }

    it 'is frozen after initialization' do
      expect(validator).to be_frozen
    end

    it 'prevents reassigning instance variables (shallow freeze)' do
      expect { validator.instance_variable_set(:@option, {}) }.to raise_error(FrozenError)
    end

    it 'deep-freezes the referenced @option object' do
      expect(validator.instance_variable_get(:@option)).to be_frozen
    end
  end
