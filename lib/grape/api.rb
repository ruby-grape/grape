  ...
  class API
    class << self
      ...
      def call(env)
        ...
        rewrite_path_info(env)
        route_set.freeze.call(env)
      end

      def rewrite_path_info(env)
        if vendors && env['HTTP_ACCEPT'] =~ /application\/vnd\.(.+)-(.+)\+(.+)/
          potential_vendor       = $1
          potential_version      = $2
          potential_content_type = $3

          if vendors && vendors.include?(potential_vendor)
            if versions && versions.include?(potential_version)
              env['api.version'] = potential_version
              path = env['PATH_INFO']
              if prefix
                path.gsub(/\/#{prefix}/, "/#{prefix}/#{potential_version}")
              else
                path = "/#{potential_version}#{path}"
              end
              env['PATH_INFO'] = path
            end
          end
      ...
      # Specify an API vendor
      #
      # This vendor is used in the Accept-Header to determine
      # which Accept-Headers can be responded to
      #
      # Accept: application/vnd.<vendor>-<version>+<contenttype>
      #
      # @example API with vendor
      ...
      #   end
      # end
      def vendor(*new_vendors, &block)
        if new_vendors.any?
          @vendors = vendors | new_vendors
          nest(block) { set(:vendor, new_vendors) }
        else
          settings[:vendor]
        end
      end
